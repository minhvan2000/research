...
const Dictionary = [
    {
        type: 'option',
        description: ['ignore', 'timestamp', 'array', 'event', 'text'],
    },
    {
        type: 'type',
        description: [
...
const Enum = ['onoff', 'day of week', 'action', 'action'];
...
const actions = {
        ...
        if (value[0]) {
            switch (value[1]) {
                ...
                case 1:
                    ...
                    state.encode[2] = arr[0];
                    break;
                case 2:
                    objectFormat.cmd = value[0];
                    objectFormat.info = new Array();
                    objectFormat.option = new Array();
                    objectFormat.description = Array.of({
                        parameter: '',
                        access: [],
                        option: [],
                        format: '',
                    ...
                    state.encode.push(...arrInfo);
                    break;
                case 3:
                    if (objectFormat.info.length) {
                        const isExist = objectFormat.info.every(
                            (elm) => elm.mode !== value[0]
                        );

                        if (isExist) {
                            objectFormat.info.push({
                                mode: value[0],
                                length: 0,
                            });
                        } else {
                            objectFormat.info = objectFormat.info.filter(
                                (val) => val.mode !== value[0]
                            );
                        }
                    } else {
                        objectFormat.info.push({ mode: value[0], length: 0 });
                    }
                    optionAccess.push(
                        ...objectFormat.info.map((val) => val.mode)
                    );

                    break;
                case 4:
                    objectFormat.option[0] = value[0];
                    break;
                case 5:
                    // objectFormat.option[0] = value[0];
                    break;
                default:
                    // this.toggleProperty(state, value);
                    break;
            }
        } else {
            encode.length = 1;
    ...
    togglePackage(state, value) {
        switch (value[2]) {
            case 0: {
                objectFormat.cmd = value[0];
                console.log(state.encode[value[1]]);
                break;
            }
            default:
                break;
    ...
    // toggleProperty(state, value) {
    //     switch (value[2]) {
    //         case 0: {
    //             if (objectFormat.info.length) {
    //                 const isExist = objectFormat.info.every(
    //                     (elm) => elm.name !== value[0]
    //                 );

    //                 if (isExist) {
    //                     objectFormat.info.push({ name: value[0], length: 0 });
    //                 } else {
    //                     objectFormat.info = objectFormat.info.filter(
    //                         (val) => val.name !== value[0]
    //                     );
    //                 }
    //             } else {
    //                 objectFormat.info.push({ name: value[0], length: 0 });
    ...
    togglePayload(state, value) {
        if (state.encode[value[1]].dataInput?.length) {
            console.log(state.encode[value[1]].dataInput[value[2]][value[3]]);
            if (value[4]) {
                const indexArr = parseInt(value[4]) - 1;

                state.encode[value[1]].dataInput[value[2]][value[3]][indexArr][
                    value[5]
                ] = value[0];
                objectFormat.description[value[2]][value[3]][indexArr][
                    value[5]
                ] = value[0];
            } else {
                state.encode[value[1]].dataInput[value[2]][value[3]] = value[0];
                objectFormat.description[value[2]][value[3]] = value[0];
            }
        }
