...
async function accessDeviceDataAndSendCommand() {
    ...
    try {
        ...
        await driver.get(`http://${process.env.APP_HOST}`);
        ...
        const rawDataID = fs.readFileSync('./list_device_ESO.csv', {
            encoding: 'utf8',
        });
...
async function findDeviceAndSendCommand(driver, item) {
    ...
    console.log(dataCsv[item][dataCsv[item].length - 1].status);
...
async function findDeviceTest(driver, item) {
    ...
    while (retryCount > 0 && !flagError) {
        try {
            ...
            const statusDevice = await driver.findElement(
                By.css('.agm-info-window-content tr:nth-child(3) td')
            );

            const statusText = await statusDevice.getText();

            if (!statusText) {
                throw Error('status Text not found');
            }

            console.log('statusText:::', statusText);

            const status = statusText.replace(/Status: /gi, '');
...
async function sendCommand(driver, item) {
    ...
    while (retryCount > 0 && !flagError) {
        try {
            ...
            dataCsv[item][dataCsv[item].length - 1].dimming =
