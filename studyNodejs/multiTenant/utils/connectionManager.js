...
import { getATenantRepo, getTenantsRepo } from '../repositories/tenant.repo.js';
import {
    getCacheConnection,
    getCacheValuesArr,
    setCacheConnection,
} from './lruCacheManager.js';
import { convertToObjectId } from './misc.js';
...
export const connectAllDb = async () => {
    const ADMIN_DB_URI = `mongodb://127.0.0.1:27017/AdminPlatformAdmin`;
...
export const getConnectionForTenant = async (tenantId) => {
    ...
    if (!connection) {
        ...
        const tenantData = await getATenantRepo(
            adminDbConnection,
            { _id: convertToObjectId(tenantId) },
            { dbUri: 1, name: 1 }
        );
